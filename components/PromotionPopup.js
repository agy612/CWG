    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <div className="relative w-full max-w-[340px] bg-card-gray rounded-[28px] border border-themed-light shadow-[0_20px_60px_rgba(0,0,0,0.8)] overflow-hidden">
                ...
                {/* Glow */}
                <div className="absolute top-0 right-0 w-48 h-48 bg-accent-soft blur-[60px] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none" />
                ...
                <div className="flex justify-between items-center px-5 pt-5 pb-4 border-b border-themed">
                    <span className="text-t-primary text-sm font-extrabold flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[16px] text-accent" style={{ fontVariationSettings: "'FILL' 1" }}>redeem</span>
                        특별 프로모션!
                    </span>
                ...
                <div className="flex flex-col items-center px-6 py-6 gap-4">
                    <div className="size-16 rounded-full bg-accent-soft flex items-center justify-center">
                        <span className="material-symbols-outlined text-[32px] text-accent" style={{ fontVariationSettings: "'FILL' 1" }}>monetization_on</span>
                    </div>
                    ...
                    <div className="w-full flex flex-col gap-2">
                        {PROMO.benefits.map((b, i) => (
                            <div key={i} className="flex items-center gap-2 bg-accent-soft rounded-xl px-4 py-3 border border-accent/20">
                                <span className="material-symbols-outlined text-[14px] text-accent" style={{ fontVariationSettings: "'FILL' 1" }}>auto_awesome</span>
                                <span className="text-accent text-sm font-semibold">{b}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    {claimed ? (
                        <div className="w-full flex items-center justify-center gap-2 py-4 bg-accent-soft rounded-2xl border border-accent/20">
                            <span className="material-symbols-outlined text-[20px] text-accent" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                            <span className="text-accent font-bold text-sm">혜택이 지급되었습니다!</span>
                        </div>
                    ) : (
                        <div className="w-full flex gap-3">
                            <button onClick={handleClaim} className="flex-1 py-3 rounded-xl bg-accent text-accent-fg font-bold text-sm active:scale-95 transition-all">
                                받기
                            </button>
